...
from k8s_utils import (abandon_lifecycle_action, cordon_node, node_exists, remove_all_pods, taint_node)
...
def _lambda_handler(env, k8s_config, k8s_client, event):
    ...
    if event['recylce_node']:
    ## grab the instance_id and get node_name to taint
        instance_id = event['recylce_node']['instance_id']
        apply_taint = event['recylce_node']['taint']
    else: 
    ## default behaviour is to listen on autoscaling lifehook for draining and eviction when an instance is terminated.
    ...
    try:
        ...
        if apply_taint:
        # taint based on input event
            taint_node(v1, node_name)
        else:
        # default behaviour for draining terminated nodes in ASG based off ASG Lifecycle Hook
